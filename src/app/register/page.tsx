...
export default function RegisterPage() {
    ...
    const inputClass = "w-full bg-background border border-border text-foreground placeholder:text-foreground/40 rounded-xl pl-10 pr-4 py-3 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/20 transition-all";
                ...
                <div className="space-y-10">
                    <div>
                        <h2 className="text-4xl xl:text-5xl font-heading font-bold text-foreground leading-tight mb-4">
                            Stop emailing.<br />
                            <span className="text-brand">Start talking.</span>
                        </h2>
                    ...
                    <div className="space-y-4">
                        {PROMISES.map((promise, i) => (
                            <motion.div key={promise} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }}
                                ...
                                <p className="text-sm text-foreground/80 leading-snug">{promise}</p>
                            </motion.div>
                        ))}
                    </div>
                </div>
                ...
                >
                    ...
                    <div className="glass rounded-3xl p-8 shadow-2xl">
                        ...
                        <form onSubmit={handleRegister} className="space-y-3.5">
                            ...
                            <div className="relative">
                                <UserIcon size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-foreground/40 pointer-events-none" />
                            ...
                            <p className="text-center text-sm text-foreground/50 pt-1">
                                ...
                                <Link href="/login" className="text-brand hover:text-brand-hover transition-colors font-medium underline underline-offset-2">Sign in</Link>
                            </p>
                        </form>
                    </div>
